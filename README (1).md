...
1. Looks up the subject and resource by ID
2. Evaluates ALL 10 policies against the request
3. Applies **deny-overrides** strategy: if any policy returns `deny`, the final decision is `deny`
4. Returns the decision plus which policies triggered (and why)
